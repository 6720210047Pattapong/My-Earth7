...
| :--- | :--- | :--- |
| **Controller** | [app/api/tasks/route.ts](file:///c:/my-earthxx/app/api/tasks/route.ts)<br>[app/api/tasks/[id]/route.ts](file:///c:/my-earthxx/app/api/tasks/%5Bid%5D/route.ts) | รับ HTTP Request (GET, POST, PATCH, DELETE), อ่าน Params และ Request Body, เรียกใช้งาน Service Layer และส่ง HTTP Response พร้อม Status Code ที่เหมาะสม (200, 201, 400, 404, 500) |
| **Service** | [lib/taskService.ts](file:///c:/my-earthxx/lib/taskService.ts) | จัดการ Business Logic, ตรวจสอบความถูกต้องของข้อมูล (Input Validation), เรียกใช้งาน Model Layer และดักจับ Prisma Error Code (`P2002`, `P2025`) เพื่อแปลงเป็น Custom Error (`ValidationError`, `NotFoundError`) |
| **Model (Prisma)** | [lib/tasks.ts](file:///c:/my-earthxx/lib/tasks.ts) | ทำหน้าที่เป็น Data Access Layer ติดต่อฐานข้อมูลผ่าน Prisma Client Singleton (`prisma.task.create`, `findMany`, `findUnique`, `update`, `delete`) |
| **Schema** | [prisma/schema.prisma](file:///c:/my-earthxx/prisma/schema.prisma) | นิยามโครงสร้างตาราง `Task`, ชนิดข้อมูลของแต่ละฟิลด์ และ Constraints ต่างๆ เช่น `@id`, `@default`, `@unique` |
